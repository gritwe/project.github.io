...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (typeof window.exercisesData === 'undefined') {
    console.error(
      'exercisesData не найден. Убедитесь, что exercises.js загружен перед этим файлом.',
    );
    return;
  }

  const { exercisesData } = window;
  ...
  function getExerciseLink(exercise) {
    ...
    const exerciseSlug = exercise.title
      .toLowerCase()
  ...
  function getExerciseImage(exercise) {
    ...
    const imageName = exercise.title
      .toLowerCase()
      .replace(/[^а-яa-z0-9\s]/g, '')
      .replace(/\s+/g, '-');
  ...
  function openMuscleGroup(muscleName, scrollToGroup = true) {
    ...
    const targetGroup = Object.keys(muscleGroupConfig).find(
      (key) =>
        key === muscleName || key.toLowerCase() === muscleName.toLowerCase(),
    );
    ...
    const groupElement = document.querySelector(
      `.muscle-group[data-muscle="${targetGroup}"]`,
    );
    if (!groupElement) {
  ...
  function createMuscleGroups() {
    ...
    const uniqueMuscleGroups = [
      ...new Set(enhancedExercises.map((ex) => ex.muscleGroup)),
    ];
    ...
    uniqueMuscleGroups.forEach((muscleName) => {
      ...
      const exercisesCount = enhancedExercises.filter(
        (ex) => ex.muscleGroup === muscleName,
      ).length;
        ...
        if (targetPanel.classList.contains('active')) {
          ...
          document.querySelectorAll('.exercises-panel').forEach((panel) => {
            if (panel !== targetPanel && panel.classList.contains('active')) {
              ...
              if (
                otherHeader &&
                otherHeader.classList.contains('group-header')
              ) {
          ...
          const { scrollHeight } = targetPanel;
  ...
  function performSearch() {
    ...
    const filteredExercises = enhancedExercises.filter(
      (exercise) =>
        ...
        (exercise.englishTitle &&
          exercise.englishTitle.toLowerCase().includes(searchTerm)),
    );
  ...
  function createExerciseCardWithHighlight(exercise, searchTerm) {
    ...
    const highlightedTitle = highlightText(exercise.title, searchTerm);
