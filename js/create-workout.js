...
class WorkoutGenerator {
  constructor(
    exercises,
    userLevel = 'Средний',
    equipment = ['Штанга', 'Гантели', 'Тренажеры', 'Без оборудования'],
  ) {
    ...
    this.equipment = this.equipment.map((eq) =>
      this.normalizeEquipmentName(eq),
    );
  ...
  createExercisePool() {
    ...
    if (this.exercisePool.length < 50) {
      ...
      if (this.equipment.includes('Турник/брусья')) {
        const bodyweightExercises = this.exercises.filter(
          (ex) =>
  ...
  getExerciseEnglishTitle(exercise) {
    ...
    if (window.exercisesData) {
      originalExercise = window.exercisesData.find(
        (ex) =>
          ex.id === exercise.id ||
  ...
  filterExercises(muscleGroup, count = 2, dayNumber = 1) {
    const availableExercises = this.exercisePoolByMuscle[muscleGroup] || [];

    let unusedExercises = availableExercises.filter(
      (ex) => !this.usedExercises.has(ex.title),
    );
    ...
    if (unusedExercises.length < count && muscleGroup !== 'Разное') {
      if (muscleGroup === 'Грудь') {
        const chestAlternatives = (
          this.exercisePoolByMuscle['Плечи'] || []
        ).filter((ex) => !this.usedExercises.has(ex.title));
        unusedExercises = [...unusedExercises, ...chestAlternatives];
      }

      if (muscleGroup === 'Спина') {
        const backAlternatives = (
          this.exercisePoolByMuscle['Предплечья'] || []
        ).filter((ex) => !this.usedExercises.has(ex.title));
    ...
    const selected = this.getRandomItems(
      unusedExercises,
      Math.min(count, unusedExercises.length),
    );
  ...
  generateDay(dayNumber, type, totalDays = 28) {
    ...
    if (
      this.equipment.includes('Турник/брусья') &&
      this.equipment.length <= 2
    ) {
...
class WorkoutApp {
    ...
    try {
      ...
      if (equipment.length === 0) {
        this.showErrorMessage(
          'Пожалуйста, выберите хотя бы один тип оборудования',
        );
        return;
      }
      ...
      this.showErrorMessage(
        `❌ Ошибка при генерации программы: ${error.message}`,
      );
    }
  }
  ...
  async displayProgram() {
    ...
    if (
      !this.currentProgram ||
      !this.currentProgram.days ||
      this.currentProgram.days.length === 0
    ) {
      output.innerHTML = `
    ...
    try {
      localStorage.setItem(
        'fitgy_selectedDay',
        this.selectedDayNumber.toString(),
      );
    } catch (e) {
  ...
  createProgramControls() {
    ...
    const saveText = this.hasSavedProgramInFirebase
      ? 'Сохранено'
      : 'Сохранить программу';
  ...
  createProgressStats() {
    const completedDays = Object.values(this.workoutStatus).filter(
      (s) => s.completed,
    ).length;
      ...
      if (window.exercisesData) {
        originalExercise = window.exercisesData.find(
          (ex) =>
            ex.id === exercise.id ||
            ex.title === exercise.title ||
            (exercise.englishTitle &&
              ex.englishTitle === exercise.englishTitle),
        );
      }
  ...
  getSetText(sets) {
    ...
    if (sets % 10 === 1 && sets % 100 !== 11) {
      ...
      [2, 3, 4].includes(sets % 10) &&
      ![12, 13, 14].includes(sets % 100)
    ) {
      word = 'подхода';
    } else {
      word = 'подходов';
    ...
    try {
      ...
      if (!user) {
        const shouldLogin = confirm(
          'Для сохранения программы необходимо войти в систему. Перейти на страницу входа?',
        );
        if (shouldLogin) {
  ...
  async toggleWorkoutCompletion() {
    try {
      ...
      if (!user) {
        const shouldLogin = confirm(
          'Для отметки тренировок необходимо войти в систему. Перейти на страницу входа?',
        );
        if (shouldLogin) {
          window.location.href = 'login.html';
        }
      ...
      const success = await authModule.updateWorkoutStatus(
        this.selectedDayNumber,
        isCompleted,
      );
