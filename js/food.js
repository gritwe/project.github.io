...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function validateInputs() {
    ...
    if (
      heightInput.value === '' ||
      heightInput.value < 140 ||
      heightInput.value > 220
    ) {
  ...
  function calculateMassData(mifflin, harris, weight) {
    ...
    const massMin = Math.round(baseCalories * 1.1);
    const massMax = Math.round(baseCalories * 1.2);
  ...
  function performCalculation() {
    ...
    const mainResults = calculateMainResults(
      gender,
      age,
      height,
      weight,
