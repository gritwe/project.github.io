...
class NutritionPlanGenerator {
  constructor() {
    ...
    this.healthyBreakfastCategories = [
      'breakfast',
      'cereal',
      'porridge',
      'dairy',
    ...
    try {
      ...
      this.recipes = this.recipes.map((recipe) => {
        ...
        const normalized = {
          ...
          protein:
            nutrition.protein || nutrition.proteins || nutrition.Protein || 0,
  ...
  setupEventListeners() {
    // Навигация
    document
      .getElementById('newPlanBtn')
      ?.addEventListener('click', () => this.showSetupSection());
    document
    ...
    document
      ...
      ?.addEventListener('click', async () => await this.savePlan());

    // Скачивание списка покупок
    document
      .getElementById('downloadShoppingListBtn')
    ...
    document.getElementById('nextWeekBtn')?.addEventListener('click', () => {
      const maxWeeks = this.planData?.settings?.duration
        ? Math.ceil(this.planData.settings.duration / 7)
        : 4;
    ...
    document
      .getElementById('correctForCaloriesBtn')
      ?.addEventListener('click', () => this.correctBJUForCalories());
    ...
    document
      .getElementById('closeAddRecipeModal')
      ?.addEventListener('click', () => {
        document.getElementById('addRecipeModal').classList.remove('active');
      });

    document
      .getElementById('closePortionModal')
      ?.addEventListener('click', () => {
  ...
  showCorrectionButtonIfNeeded() {
    const calories =
      parseInt(document.getElementById('targetCalories')?.value) || 0;
    const protein =
      parseInt(document.getElementById('targetProtein')?.value) || 0;
  ...
  correctBJUForCalories() {
    const targetCalories =
      parseInt(document.getElementById('targetCalories')?.value) || 2000;
    const currentProtein =
      parseInt(document.getElementById('targetProtein')?.value) || 100;
    const currentFat =
      parseInt(document.getElementById('targetFat')?.value) || 70;
    const currentCarbs =
      parseInt(document.getElementById('targetCarbs')?.value) || 250;

    const currentCalories =
      currentProtein * 4 + currentFat * 9 + currentCarbs * 4;
    ...
    if (Math.abs(finalDiff) <= 10) {
      showCustomAlert(
        `БЖУ скорректированы. Теперь: ${Math.round(finalCalories)} ккал`,
        'success',
      );
    } else {
      showCustomAlert(
        `БЖУ частично скорректированы. Разница: ${Math.round(finalDiff)} ккал`,
        'info',
      );
    }
    ...
    try {
      ...
      if (savedPlan) {
        if (!this.planData) {
          ...
          this.planData.weeks[this.currentWeek] =
            savedPlan.weeks[this.currentWeek];
        }
        this.renderPlan();
      }
    } catch (error) {
  ...
  generatePlan() {
    ...
    const mealCount = parseInt(
      document.querySelector('input[name="mealCount"]:checked')?.value || '3',
    );
    const duration = parseInt(
      document.querySelector('input[name="duration"]:checked')?.value || '28',
    );
    ...
    for (let week = 1; week <= totalWeeks; week++) {
      this.planData.weeks[week] = this.generateUltraDiverseWeek(
        week,
        goals,
        mealCount,
        dietType,
  ...
  getSelectedRestrictions() {
    const restrictions = [];
    document
      .querySelectorAll('.restriction-checkbox input:checked')
      .forEach((cb) => {
    ...
    switch (mealCount) {
      case 3:
        return {
          ...
          dinner: { name: 'Ужин', time: '19:00', weight: 0.3 },
        };
      case 4:
        return {
          breakfast: { name: 'Завтрак', time: '8:00', weight: 0.2 },
      ...
      case 5:
        return {
          breakfast: { name: 'Завтрак', time: '8:00', weight: 0.2 },
          snack1: { name: 'Перекус', time: '11:00', weight: 0.1 },
          lunch: { name: 'Обед', time: '14:00', weight: 0.3 },
      ...
      default:
        return {
          breakfast: { name: 'Завтрак', time: '8:00', weight: 0.25 },
          lunch: { name: 'Обед', time: '13:00', weight: 0.4 },
  ...
  generateUltraDiverseMeal(
    ...
    const dishTypes = this.selectDiverseDishTypes(
      mealType,
      dietType,
      week,
      day,
    ...
    const currentNutrition = { calories: 0, protein: 0, fat: 0, carbs: 0 };

    // Подбираем рецепты для каждого типа блюда
    for (const dishType of dishTypes) {
      const recipe = this.findUltraDiverseRecipe(
        dishType,
        mealGoals,
        currentNutrition,
        dietType,
      ...
      if (recipe) {
        const portion = this.calculateOptimalPortion(
          recipe,
          dishType,
          mealGoals,
          currentNutrition,
  ...
  selectDiverseDishTypes(mealType, dietType, week, day) {
    const variations =
      this.dishTypeVariations[mealType] || this.dishTypeVariations.breakfast;
    ...
    if (availablePrimary.length > 0) {
      selectedTypes.push(
        this.selectLeastUsedType(availablePrimary, mealType, week, day),
      );
    }
    ...
    if (availableSecondary.length > 0 && Math.random() > 0.3) {
      selectedTypes.push(
        this.selectLeastUsedType(availableSecondary, mealType, week, day),
      );
    }
    ...
    if (availableTertiary.length > 0 && Math.random() > 0.7) {
      selectedTypes.push(
        this.selectLeastUsedType(availableTertiary, mealType, week, day),
      );
    }
  ...
  findUltraDiverseRecipe(
    ...
    // Фильтруем по типу
    let filtered = this.recipes.filter(
      (recipe) =>
        this.isRecipeOfType(recipe, dishType) &&
        this.isRecipeAllowed(
          recipe,
          this.planData?.settings?.restrictions || [],
          dietType,
        ),
    );
    ...
    filtered = filtered
      .map((recipe) => {
        const score = this.calculateUltraDiversityScore(
          recipe,
          dishType,
          mealGoals,
          currentNutrition,
  ...
  calculateOptimalPortion(
    ...
    if (
      dietType === 'keto' &&
      ['cereal', 'rice', 'pasta', 'baking'].includes(dishType)
    ) {
    ...
    const minPortion =
      dishType === 'nuts' ? 30 : dishType === 'protein_bar' ? 40 : 100;
    const maxPortion =
      dishType === 'soup' ? 400 : dishType === 'vegetables' ? 350 : 300;
  ...
  addBalancingDishes(
    ...
    if (priorities.includes('protein')) {
      dishType = this.getRandomFromArray([
        'protein_bar',
        'eggs',
        'dairy',
        'poultry',
  ...
  determineRecipeType(recipe) {
    ...
    const typeMap = [
      ...
      {
        keywords: ['каша', 'овсян', 'гречнев', 'рисовая каша'],
        type: 'porridge',
      },
      ...
      {
        keywords: ['гарнир', 'паста', 'макарон', 'рис ', 'греч ', 'картофель'],
        type: 'side_dish',
      },
      { keywords: ['куриц', 'индейк', 'утк'], type: 'poultry' },
      { keywords: ['рыб', 'лосос', 'тунец', 'треск'], type: 'fish' },
      {
        keywords: ['овощ', 'брокколи', 'цветная', 'морковь', 'капуст'],
        type: 'vegetables',
      },
    ];
  ...
  isDishTypeAllowed(dishType, dietType) {
    ...
    for (const restriction of dietRestrictions) {
      if (
        dishTypeMapping[restriction] &&
        dishTypeMapping[restriction].includes(dishType)
      ) {
        return false;
      }
    }
  ...
  isRecipeAllowed(recipe, restrictions, dietType) {
    ...
    switch (dietType) {
      case 'vegetarian':
        if (this.hasMeat(ingredients) || this.hasFish(ingredients))
          return false;
        break;
      case 'vegan':
        ...
        break;
      case 'keto':
        if (recipe.nutrition.carbs > 10 && recipe.nutrition.calories > 100)
          return false;
        break;
      case 'high_protein':
        if (recipe.nutrition.protein < 15 && recipe.nutrition.calories > 150)
          return false;
        break;
      case 'low_fat':
        if (recipe.nutrition.fat > 10 && recipe.nutrition.calories > 150)
          return false;
        break;
    }
  ...
  addRecipeToHistory(recipeName, week, day) {
    ...
    if (
      this.usedRecipesHistory[weekKey] &&
      this.usedRecipesHistory[weekKey][dayKey]
    ) {
      if (!this.usedRecipesHistory[weekKey][dayKey].includes(recipeName)) {
      ...
      this.recipeUsageCount[recipeName] =
        (this.recipeUsageCount[recipeName] || 0) + 1;
  ...
  showEnhancedVarietyStats() {
      ...
      Object.values(week.days || {}).forEach((day) => {
        Object.values(day.meals || {}).forEach((meal) => {
          meal.recipes.forEach((recipe) => {
            ...
            totalRecipes++;
            recipeFrequency[recipe.name] =
              (recipeFrequency[recipe.name] || 0) + 1;
          });
        });
      });
    });
    ...
    const varietyPercent =
      totalRecipes > 0
        ? Math.round((uniqueRecipes.size / totalRecipes) * 100)
        : 0;
    ...
    if (statsElement) {
      ...
      document
        .getElementById('regenerateVarietyBtn')
        .addEventListener('click', () => {
          this.regenerateWithVariety();
        });

      document
        .getElementById('showVarietyDetails')
        .addEventListener('click', () => {
  ...
  regenerateWithVariety() {
    ...
    if (
      confirm(
        'Сгенерировать план с полностью новыми блюдами? Текущие блюда будут заменены.',
      )
    ) {
      const { goals } = this.planData;
      const { settings } = this.planData;
  ...
  renderEmptyPlan() {
    ...
    document
      .getElementById('generateFromEmpty')
      .addEventListener('click', () => {
        this.generatePlan();
      });
  ...
  createMealElement(meal, mealType) {
    ...
    if (meal.recipes.length === 0) {
      ...
      dishesHTML = meal.recipes
        .map((recipe, index) => {
          ...
          if (usageCount <= 1) {
            noveltyBadge =
              '<span class="novelty-badge new" title="Новое блюдо">🆕</span>';
          } else if (usageCount <= 3) {
            noveltyBadge =
              '<span class="novelty-badge rare" title="Редкое блюдо">🔸</span>';
          }
    ...
    const calorieProgress =
      meal.goals.calories > 0
        ? (meal.actual.calories || 0) / meal.goals.calories
        : 0;
  ...
  updateDaySummaryValues(totalCalories, totalProtein, totalFat, totalCarbs) {
    ...
    document.getElementById('actualCalories').textContent =
    ...
    const proteinPercent =
      goals.protein > 0
        ? Math.min(100, (totalProtein / goals.protein) * 100)
        : 0;
    const fatPercent =
      goals.fat > 0 ? Math.min(100, (totalFat / goals.fat) * 100) : 0;
    const carbsPercent =
      goals.carbs > 0 ? Math.min(100, (totalCarbs / goals.carbs) * 100) : 0;
  ...
  removeRecipeFromMeal(mealType, recipeIndex) {
    if (
      !this.planData ||
      !this.planData.weeks[this.currentWeek]?.days[this.currentDay]?.meals[
        mealType
      ]
    ) {
      return;
  ...
  removeRecipeFromHistory(recipeName, week, day) {
    ...
    if (
      ...
      const index =
        this.usedRecipesHistory[weekKey][dayKey].indexOf(recipeName);
      if (index > -1) {
  ...
  showAddRecipeModal(mealType) {
    ...
    relevantCategories = relevantCategories.filter((cat) =>
      this.isDishTypeAllowed(cat, dietType),
    );
  ...
  filterRecipesByType(type, mealType) {
    ...
    recipesList.innerHTML =
      '<div class="loading"><i class="fas fa-spinner fa-spin"></i> Загрузка рецептов...</div>';
    ...
    setTimeout(() => {
      ...
      if (mealType === 'breakfast') {
        filtered = filtered.filter((recipe) =>
          this.isRecipeHealthyForBreakfast(recipe),
        );
      }
  ...
  isRecipeHealthyForBreakfast(recipe) {
    ...
    if (recipe.nutrition.carbs > 60 && recipe.nutrition.protein < 10)
      return false;
  ...
  displayRecipesList(recipes, mealType) {
    ...
    if (recipes.length === 0) {
      recipesList.innerHTML =
        '<div class="empty-meal"><p>Нет подходящих рецептов</p></div>';
      return;
    }
    ...
    recipes.forEach((recipe) => {
      ...
      if (
        ...
        warning =
          '<div class="recipe-warning">⚠️ Не рекомендуется для завтрака</div>';
      }
  ...
  searchRecipes(query) {
    if (!query.trim()) {
      ...
      if (activeCategory) {
        this.filterRecipesByType(
          activeCategory.dataset.type,
          this.selectedMealType,
        );
      } else {
        this.displayRecipesList(
          this.recipes.slice(0, 50),
          this.selectedMealType,
        );
      }
      return;
    ...
    const recipesList = document.getElementById('recipesList');
    recipesList.innerHTML =
      '<div class="loading"><i class="fas fa-spinner fa-spin"></i> Поиск рецептов...</div>';
    ...
    setTimeout(() => {
      ...
      let filtered = this.recipes.filter((recipe) => {
        ...
        return (
          nameMatch || categoryMatch || ingredientMatch || descriptionMatch
        );
      });
  ...
  addRecipeToMeal(recipe) {
    ...
    if (
      ...
      if (
        !confirm('Это блюдо не рекомендуется для завтрака. Добавить все равно?')
      ) {
        return;
      }
    }

    const { mealCount } = this.planData.settings;
    const mealTypes = this.getMealTypes(mealCount);
    ...
    const currentNutrition = meal
      ? { ...meal.actual }
      : { calories: 0, protein: 0, fat: 0, carbs: 0 };
    const mealGoals = meal
      ? meal.goals
      : {
          calories: Math.round(
            this.planData.goals.calories * (mealInfo?.weight || 0.25),
          ),
          protein: Math.round(
            this.planData.goals.protein * (mealInfo?.weight || 0.25),
          ),
          fat: Math.round(this.planData.goals.fat * (mealInfo?.weight || 0.25)),
          carbs: Math.round(
            this.planData.goals.carbs * (mealInfo?.weight || 0.25),
          ),
        };
    ...
    const portion = this.calculateOptimalPortion(
      ...
      mealGoals,
      currentNutrition,
      this.planData.settings.dietType,
    );
    const scaledRecipe = this.createScaledRecipe(recipe, portion, dishType);
  ...
  fillMealCaloriesBalanced(mealType) {
    ...
    if (!recipe) {
      showCustomAlert(
        'Не удалось найти подходящее дополнительное блюдо',
        'warning',
      );
      return;
    }
  ...
  showPortionModal(recipe, mealType, recipeIndex) {
    ...
    document.getElementById('portionCalories').textContent =
      currentRecipe.scaledNutrition.calories;
    document.getElementById('portionProtein').textContent =
      `${currentRecipe.scaledNutrition.protein}г`;
    document.getElementById('portionFat').textContent =
      `${currentRecipe.scaledNutrition.fat}г`;
    document.getElementById('portionCarbs').textContent =
      `${currentRecipe.scaledNutrition.carbs}г`;
    ...
    portionSlider.addEventListener('input', () => {
      ...
      document.getElementById('portionProtein').textContent =
        `${scaledNutrition.protein}г`;
      document.getElementById('portionFat').textContent =
        `${scaledNutrition.fat}г`;
      document.getElementById('portionCarbs').textContent =
        `${scaledNutrition.carbs}г`;
    });

    document.getElementById('portionSaveBtn').onclick = () => {
      const newPortion = parseInt(portionSlider.value);

      const scaledRecipe = this.createScaledRecipe(
        recipe,
        newPortion,
  ...
  parseIngredient(ingredientStr) {
    try {
      ...
      if (
        cleanStr.toLowerCase().includes('по вкусу') ||
  ...
  generateShoppingList() {
    ...
    const preCleanName = (name) => {
      ...
      cleaned = cleaned.replace(
        /^\d+([.,]\d+)?\s*(гр?|кг|мл|л|шт|%|ст\.?|ч\.?л\.?|ст\.?л\.?)/,
        '',
      );

      // Удаляем числовые суффиксы с единицами измерения
      cleaned = cleaned.replace(
        /\s+\d+([.,]\d+)?\s*(гр?|кг|мл|л|шт|%|ст\.?|ч\.?л\.?|ст\.?л\.?)$/,
        '',
      );
      ...
      cleaned = cleaned.replace(
        /\s+\d+\s*(шт|яблок[ао]?|помидор[аы]?|огурц[аы]?|морков[ьи]?|перц[аы]?)$/,
        '',
      );
    ...
    const normalizeIngredientName = (name) => {
      ...
      normalized = normalized.replace(
        /\s*(по вкусу|для украшения|для подачи|для смазывания|для начинки|для коржей|для карамели)\s*/g,
        '',
      );
      ...
      const isLiquid = (str) =>
        /(вода|молоко|кефир|сок|уксус|бульон|соус|масло|вино)/.test(str);
      const isPowder = (str) =>
        /(мука|сахар|соль|пудра|сахарная|пудра|карри|кофе|чай)/.test(str);
      const isSolid = (str) =>
        /(сыр|творог|йогурт|шоколад|орех|мясо|филе|колбаса|сосиска|хлеб|лаваш)/.test(
          str,
        );
      const isFruitVegetable = (str) =>
        /(яблок|помидор|огурец|морков|перец|виноград|фасоль|гриб|шампиньон)/.test(
          str,
        );
      ...
      if (isFruitVegetable(normalized)) {
        normalized = normalized.replace(
          /\s+\d+\s*(шт|яблок|помидор|огурец|морков|перец)/g,
          '',
        );
      ...
      normalized = normalized.replace(
        /\s+\d+([.,]\d+)?\s*(гр?|кг|мл|л|шт|%|ст|ч\.л|ст\.л)/g,
        '',
      );
      ...
      for (const [key, value] of Object.entries(standardNames)) {
        if (normalized.includes(key) && key.length > 2) {
          ...
          if (
            key === 'масло' &&
            (normalized.includes('сливочное') ||
              normalized.includes('растительное'))
          ) {
          ...
          if (
            key === 'сыр' &&
            (normalized.includes('адыгейск') ||
              normalized.includes('пармезан') ||
          ...
          if (
            key === 'мука' &&
            (normalized.includes('пшенич') ||
              normalized.includes('рисов') ||
          ...
          if (
            key === 'перец' &&
            (normalized.includes('болгарск') ||
              normalized.includes('красн') ||
              normalized.includes('черн'))
          ) {
            continue; // Пропускаем, так как это будет обработано специальными ключами
          }
    ...
    const cleanNumericParts = (str) => {
      ...
      return str
        .replace(
          /\s+\d+([.,]\d+)?\s*(гр?|кг|мл|л|шт|%|ст|ст\.|ст\.л|ч\.|ч\.л)/g,
          '',
        )
        .replace(
          /\s+\d+\s*(шт|яблок[ао]?|помидор[аы]?|огурц[аы]?|морков[ьи]?|перц[аы]?)/g,
          '',
        )
        .replace(/\s+\d+([.,]\d+)?/g, '')
        .replace(/\s+/g, ' ')
              ...
              if (parsed) {
                ...
                if (ingredientMap.has(key)) {
                  ...
                  if (existing.unit === standardUnit) {
                    ...
                    (existing.unit === 'г' && standardUnit === 'кг') ||
                    ...
                    (existing.unit === 'мл' && standardUnit === 'л') ||
    ...
    const postProcessGroupedIngredients = (ingredientsArray) => {
      ...
      ingredientsArray.forEach((item) => {
        ...
        if (key.includes('яйц')) {
          ...
          if (existing) {
            ...
            if (existing.unit === 'шт' && item.unit === 'шт') {
              ...
              // Конвертируем в стандартные единицы
              const amount1 =
                existing.unit === 'шт' ? existing.amount : existing.amount;
        ...
        // Для молока - всегда группируем как "Молоко"
          ...
          key.includes('молоко') &&
          !key.includes('миндальн') &&
          !key.includes('кокосов')
        ) {
  ...
  categorizeIngredient(name) {
    ...
    // Мясо и птица
    if (
      /(курин|индейк|утк|гус|перепел|фазан|кролик|заяц|оленин|телятин|конин|баран|свинин|говядин|мясн|фарш|колбас|сосиск|ветчин|бекон|карбонад|сало|грудк|окорок|бедр|голен|крыл|стейк|отбивн|шашлык)/.test(
        nameLower,
      )
    ) {
    ...
    // Рыба и морепродукты
    if (
      /(лосос|семг|форел|тунец|сельд|скумбр|окун|карп|щук|судак|камбал|палтус|треск|минта|горбуш|кет|икра|краб|кревет|кальмар|миди|гребешок|осьминог|каракатиц|устриц|омар|лобстер|рыб|филе рыб|пангасиус)/.test(
        nameLower,
      )
    ) {
    ...
    // Молочные продукты
    if (
      /(молок|кефир|йогурт|ряженк|простокваш|сметан|творог|сыр|брынз|адыгейск|моцарел|пармезан|чеддер|сулугун|сливк|масло сливочн|маргарин|спред|топлен|творожный сыр|плавленый сыр|сливочный сыр|филадельфия)/.test(
        nameLower,
      )
    ) {
    ...
    // Овощи
    if (
      /(авокадо|баклажан|перец болгарск|перец сладк|зелен|петрушк|укроп|кинз|базилик|тимьян|мята|щавель|шпинат|руккол|салат|спарж|капуст|картофель|морков|свекл|редьк|редис|реп|лук|чеснок|помидор|томат|огур|кабачк|тыкв|сельдерей|оливк|маслин|артишок|брокколи|цветная капуст|кабачок|цуккини|патиссон|редиска)/.test(
        nameLower,
      )
    ) {
      return '🥦 Овощи';
    ...
    // Фрукты и ягоды
    if (
      /(апельсин|мандарин|грейпфрут|помело|лайм|лимон|гранат|виноград|арбуз|дын|ананас|манго|папай|гуав|маракуй|финик|инжир|хурм|фейхоа|личи|рамбутан|дуриан|питай|яблок|груш|слив|абрикос|персик|нектарин|вишн|черешн|клубник|земляник|малин|ежевик|черник|голубик|брусник|клюкв|облепих|рябин|калин|киви|банан)/.test(
        nameLower,
      )
    ) {
    ...
    if (
      /(гречк|рис|овсян|пшен|перлов|ячмен|кукурузн|манн|мука|круп|хлопь|отруб|зерн|злак|семен|семечк|кунжут|лен|льнян|чиа|подсолнечн|тыквенн|кедров|арахис|фундук|миндал|кешью|фисташк|мак|нут|фасол|горох|чечевиц|боб|соя|киноа|булгур|кус-кус|полба|амарант|теф|сорго|бобы|горошек)/.test(
        nameLower,
      )
    ) {
    ...
    // Хлеб и выпечка
    if (
      /(хлеб|булка|батон|лаваш|лепешк|пит|тортиль|тест|сухар|гренк|крекер|печенье|пряник|бублик|баранк|сушк|вафл|кекс|маффин|пирог|пирожок|пончик|булочк|рогалик|круасан|бриош)/.test(
        nameLower,
      )
    ) {
    ...
    if (
      /(соль|перец черн|перец красн|перец бел|сахар|ванил|кориц|имбир|куркум|карри|паприк|чили|мускат|лавров|базилик|орегано|тимьян|розмарин|майоран|чабер|эстрагон|шалфей|горчиц|кетчуп|майонез|соус|уксус|разрыхлитель|сода|дрожж|желатин|агар|пектин|крахмал|приправа|специ|пряност)/.test(
        nameLower,
      )
    ) {
    ...
    // Сладости и орехи
    if (
      /(шоколад|конфет|печенье|пряник|зефир|мармелад|пастил|халв|нуга|грильяж|щербет|мороженое|торт|пирожное|кекс|вафл|бисквит|крек|орех|миндал|кешью|фисташк|грецк|фундук|арахис|семечк|семян|изюм|кураг|чернослив|финик)/.test(
        nameLower,
      )
    ) {
    ...
    if (
      /(масло оливков|масло подсолнечн|масло растительн|масло кокосов|кокосовое масло|оливковое масло|подсолнечное масло|растительное масло|масло льнян|масло кунжутн|масло авокадо|жир)/.test(
        nameLower,
      )
    ) {
      return '🛒 Прочее';
    ...
    // Напитки и жидкости
    if (
      /(вода|бульон|сок|компот|морс|кисель|чай|кофе|какао|напиток|ликер|вино|пиво|алкоголь|минералка|газировка|лимонад)/.test(
        nameLower,
      )
    ) {
      return '🛒 Прочее';
  ...
  categorizeIngredient(name) {
    ...
    // Мясо и птица
    if (
      /(индейка|куриц|мясн|фарш|свинин|говядин|баран|телятин|конин|оленин|кролик|заяц|фазан|перепел|утк|гус|бройлер|филе|грудка|окорок|бедро|голень|крыло|колбас|сосиск|ветчин|бекон|сало|карбонад)/i.test(
        cleanName,
      )
    ) {
      return '🥩 Мясо и птица';
    ...
    // Рыба и морепродукты
    if (
      /(лосос|семг|форел|тунец|сельд|скумбр|окун|карп|щук|судак|камбал|палтус|треск|минта|горбуш|кет|икра|краб|кревет|кальмар|миди|гребешок|осьминог|каракатиц|устриц|омар|лобстер|рыб)/i.test(
        cleanName,
      )
    ) {
    ...
    // Молочные продукты
    if (
      /(молок|кефир|йогурт|ряженк|простокваш|сметан|творог|сыр|брынз|адыгейск|моцарел|пармезан|чеддер|сулугун|сливк|масло сливочн|маргарин|спред|топлен)/i.test(
        cleanName,
      )
    ) {
      return '🥛 Молочные продукты';
    ...
    // Овощи
    if (
      /(авокадо|баклажан|перец|зелен|петрушк|укроп|кинз|базилик|тимьян|мята|щавель|шпинат|руккол|салат|спарж|капуст|картофель|морков|свекл|редьк|редис|реп|лук|чеснок|помидор|томат|огур|кабачк|тыкв|сельдерей|оливк|маслин|артишок)/i.test(
        cleanName,
      )
    ) {
      return '🥦 Овощи';
    ...
    // Фрукты и ягоды
    if (
      /(апельсин|мандарин|грейпфрут|помело|лайм|лимон|гранат|виноград|арбуз|дын|ананас|манго|папай|гуав|маракуй|финик|инжир|хурм|фейхоа|личи|рамбутан|дуриан|питай|яблок|груш|слив|абрикос|персик|нектарин|вишн|черешн|клубник|земляник|малин|ежевик|черник|голубик|брусник|клюкв|облепих|рябин|калин|киви|банан)/i.test(
        cleanName,
      )
    ) {
      return '🍎 Фрукты и ягоды';
    ...
    // Крупы, злаки и бобовые
    if (
      /(гречк|рис|овсян|пшен|перлов|ячмен|кукурузн|манн|мука|круп|хлопь|отруб|зерн|злак|семен|семечк|кунжут|лен|льнян|чиа|подсолнечн|тыквенн|кедров|арахис|фундук|миндал|кешью|фисташк|мак|нут|фасол|горох|чечевиц|боб|соя|киноа|булгур|кус-кус|полба|амарант|теф|сорго)/i.test(
        cleanName,
      )
    ) {
      return '🍚 Крупы, злаки и бобовые';
    ...
    // Хлеб и выпечка
    if (
      /(хлеб|булка|батон|лаваш|лепешк|пит|тортиль|тест|сухар|гренк|крекер|печенье|пряник|бублик|баранк|сушк|вафл|кекс|маффин|пирог|пирожок|пончик|булочк|рогалик|круасан|бриош)/i.test(
        cleanName,
      )
    ) {
      return '🍞 Хлеб и выпечка';
    ...
    // Специи, приправы и соусы
    if (
      /(соль|перец|сахар|ванил|кориц|имбир|куркум|карри|паприк|чили|мускат|лавров|базилик|орегано|тимьян|розмарин|майоран|чабер|эстрагон|шалфей|горчиц|кетчуп|майонез|соус|уксус|разрыхлитель|сода|дрожж|желатин|агар|пектин|крахмал)/i.test(
        cleanName,
      )
    ) {
      return '🧂 Специи, приправы и соусы';
  ...
  async savePlan(silent = false) {
    ...
    try {
      ...
      if (!user) {
        if (!silent)
          showCustomAlert(
            'Для сохранения плана необходимо авторизоваться',
            'warning',
          );
        return;
      }
      ...
      localStorage.setItem(
        `mealPlan_${user.uid}_week${this.currentWeek}`,
        JSON.stringify(this.planData),
      );
    } catch (error) {
      if (!silent)
        showCustomAlert(
          `Ошибка при сохранении плана: ${error.message}`,
          'error',
        );
    }
  }
  ...
  showBJUHelp(type) {
    const helpTexts = {
      calories:
        'Рекомендуемая калорийность зависит от вашего веса, роста, возраста, пола и уровня активности. Средняя норма: 2000-2500 ккал/день.',
      protein:
        'Белки: 1.5-2.2г на кг веса. Для похудения: 2-2.4г, для набора массы: 1.6-1.8г, для поддержания: 1.2-1.5г.',
      fat: 'Жиры: 0.8-1г на кг веса. 20-35% от общей калорийности. Важны для гормонов и усвоения витаминов.',
      carbs:
        'Углеводы рассчитываются исходя из оставшихся калорий после учета белков и жиров. 45-65% от общей калорийности.',
    };
