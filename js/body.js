...
function resetHighlight() {
    if (highlightedObject) {
        console.log("[Reset] Сброс подсветки для:", highlightedObject.name || "текущего объекта");
        highlightedObject.traverse(obj => {
            if (obj.isMesh) {
                const oldMat = originalMaterials.get(obj);
                if (oldMat && oldMat.isMaterial) {
                    obj.material = oldMat;
                }
            }
        });
...
/**
 * Основная функция подсветки объекта
 * @param {THREE.Object3D} object - Объект из сцены
 * @param {string} key - Ключ из словаря (например, 'Object_1')
 */
function highlightObject(object, key) {
    console.log(`%c[Highlight] Попытка подсветки: ${key}`, "color: #00ff00; font-weight: bold;");

    // 1. Проверка блокировки
    if (isModelLocked) {
        console.warn("[Highlight] Модель заблокирована кнопкой.");
        showInfoPanel(key);
        highlightListItem(key);
        return;
    }

    // 2. Если нажали на уже подсвеченный объект — сбрасываем всё
    if (object && highlightedObject === object) {
        console.log("[Highlight] Повторный клик — сброс.");
        resetHighlight();
        return;
    }

    // 3. Очищаем предыдущую подсветку
    resetHighlight();
    ...
    if (object) {
        ...
        object.traverse(child => {
            if (child.isMesh) {
                ...
                // Сохраняем оригинальный материал, если еще не сохраняли
                if (!originalMaterials.has(child)) {
                    originalMaterials.set(child, child.material);
                }

                try {
                    // Клонируем родной материал объекта
                    // Это решает ошибку "Multiple instances of Three.js" и "onBeforeRender"
                    const highlightMat = child.material.clone();
                    
                    // Устанавливаем ярко-красный цвет
                    if (highlightMat.color) highlightMat.color.set(0xff4444);
                    
                    // Добавляем свечение (emissive)
                    if (highlightMat.emissive) {
                        highlightMat.emissive.set(0x880000);
                    ...
                    // Сообщаем движку, что нужно обновить шейдер
                    child.material.needsUpdate = true;
                } catch (err) {
                    console.error(`[Highlight] Ошибка материала на ${child.name}:`, err);
                }
            }
        });

        highlightedObject = object;
        console.log(`[Highlight] Мешей подсвечено: ${meshCount}`);

        // 4. Фокусировка камеры (с исправлением ошибки n.trim)
        focusCameraOnObject(object);

        // 5. ПРИНУДИТЕЛЬНОЕ ОБНОВЛЕНИЕ КАДРА
        // Это заменяет необходимость "кликать по модели", чтобы она покрасилась
        if (viewer && typeof viewer.queueRender === 'function') {
            viewer.queueRender();
        }
    }

    // 6. Обновление интерфейса (Панель и Список)
    if (key) {
        showInfoPanel(key);
        highlightListItem(key);
    }
...
/**
 * Исправленная функция фокусировки камеры
 */
function focusCameraOnObject(object) {
    ...
    try {
        ...
        // ЛОГ — запусти один раз и посмотри цифры
        console.log(`[Camera] center=`, center, `size=`, size);

        const maxDim = Math.max(size.x, size.y, size.z);
        const cameraDistance = Math.max(maxDim * 2.2, 0.3);

        const thetaRad = Math.atan2(center.x, center.z);
        const thetaDeg = (thetaRad * 180 / Math.PI + 360) % 360;

        // Подставь реальное значение maxY из лога вместо 1.7
        const modelMaxY = 1.7; // <-- ПОМЕНЯЙ под свою модель
        const relativeY = center.y / modelMaxY;
        const phiDeg = relativeY > 0.75 ? 60 : relativeY < 0.25 ? 95 : 78;

        const targetString = `${center.x.toFixed(3)}m ${center.y.toFixed(3)}m ${center.z.toFixed(3)}m`;
        const orbitString  = `${thetaDeg.toFixed(1)}deg ${phiDeg}deg ${cameraDistance.toFixed(3)}m`;

        console.log(`[Camera] orbit="${orbitString}" target="${targetString}"`);

        viewer.cameraTarget = targetString;
        viewer.cameraOrbit  = orbitString;
        if (viewer.queueRender) viewer.queueRender();

    } catch(e) {
        console.error("[Camera] Ошибка:", e);
    }
}
...
function findObjectInSceneByName(name) {
    if (!scene) {
        console.error("[Search] Сцена не инициализирована!");
        return null;
    }

    console.log(`[Search] Ищем объект: ${name}`);

    // 1. Попытка прямого поиска
    let found = scene.getObjectByName(name);
    if (found) {
        console.log(`[Search] Найдено через getObjectByName: ${name}`);
        return found;
    }
    ...
    if (candidate) {
        console.log(`[Search] Найдено через traverse: ${name}`);
    } else {
        console.warn(`[Search] Объект ${name} НЕ НАЙДЕН в сцене.`);
    }
...
function doRaycast(e) {
    ...
    // Идём вверх по иерархии, пока не найдём ключ из словаря
    let obj = hits[0].object;
    let dictKey = null;

    while (obj) {
        if (musclesDictionary[obj.name]) {
            dictKey = obj.name;
            break;
        }
        obj = obj.parent;
    ...
    if (!dictKey) {
        console.warn("[Raycast] Клик не попал ни в один объект из словаря. Имя меша:", hits[0].object.name);
        return;
    }

    highlightObject(obj, dictKey);
}
// ===== Список мышц =====
function createMuscleList() {
    ...
    for (const key in musclesDictionary) {
        ...
        // Внутри функции createMuscleList, где создаются items:
        item.addEventListener('click', async () => {
            console.log(`%c[UI] Клик по списку: ${key}`, "background: #222; color: #bada55");
            
            if (!scene) {
                console.log("[UI] Сцена не готова, пробуем получить доступ...");
                getSceneAndCamera();
            }
            ...
            if (!found) {
                console.log("[UI] Объект не найден сразу, запускаем ожидание...");
                found = await waitForObject(key);
            }

            if (found) {
                highlightObject(found, key);
            } else {
                console.error(`[UI] Не удалось подсветить ${key}: объект отсутствует в модели.`);
                // Показываем хотя бы инфо из словаря
                showInfoPanel(key);
                highlightListItem(key);
            }
        });
...
function getSceneAndCamera() {
    ...
    // Способ 2: прямые свойства (некоторые версии)
    ...
    // Способ 3: через renderer (последний шанс)
    if (viewer.renderer?.shadowMap) {
        try {
            const threeScene = viewer[
                Object.keys(viewer).find(k => viewer[k]?.isScene)
            ];
            if (threeScene) { scene = threeScene; return true; }
        } catch(e) {}
    }
