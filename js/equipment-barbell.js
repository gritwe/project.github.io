...
function updatePageTitle() {
  ...
  if (subtitle) {
    subtitle.innerHTML +=
      ' <span style="color:#66a3ff;font-size:0.9em">(Используется с древнеегипетских времен)</span>';
  }
}
...
function animateBarbellCards() {
  const cards = document.querySelectorAll(
    '.type-card, .exercise-card, .characteristic-item',
  );
...
async function loadBarbellData() {
  try {
    ...
    const barbellData = {
      title: 'Штанги',
      history:
        'Первые упоминания появились в Древнем Египте, современный вид сформировался в Германии в середине 19 века',
...
export { initBarbellPage, loadBarbellData };
